...
({
    ...
    setScalesDomain: function (scales, data, series, svg, options) {
        ...
        if ((series.filter(function (s) { return s.axis === "y" && s.visible !== false; })).length > 0) {
        ...
        if ((series.filter(function (s) { return s.axis === "y2" && s.visible !== false; })).length > 0) {
    ...
    getVerticalDomain: function (options, data, series, key) {
        ...
        mySeries = series.filter(function (s) { return s.axis === key && s.visible !== false; });
        domain = this.yExtent(series.filter(function (s) { return s.axis === key && s.visible !== false; }), data, options.stacks.filter(function (stack) { return stack.axis === key; }));
        if (o.type === "log") {
    ...
    yExtent: function (series, data, stacks) {
        ...
        groups = [];
        stacks.forEach(function (stack) { return groups.push(stack.series.map(function (id) { return (series.filter(function (s) { return s.id === id; }))[0]; })); });
        ...
        groups.forEach(function (group) {
            ...
            minY = Math.min(minY, d3.min(data, function (d) { return group.reduce((function (a, s) { return Math.min(a, d[s.y]); }), Number.POSITIVE_INFINITY); }));
            return maxY = Math.max(maxY, d3.max(data, function (d) { return group.reduce((function (a, s) { return a + d[s.y]; }), 0); }));
        });
        if (minY === maxY) {
            if (minY > 0) {
                return [0, minY * 2];
    ...
    xExtent: function (data, key) {
        var from, to, _ref;
        _ref = d3.extent(data, function (d) { return d[key]; }), from = _ref[0], to = _ref[1];
        if (from === to) {
            if (from > 0) {
                return [0, from * 2];
            }
