...
({
    ...
    getDefaultMargins: function () { return ({
        top: 20,
        right: 50,
        bottom: 60,
        left: 50
    }); },
    getDefaultThumbnailMargins: function () { return ({
        top: 1,
        right: 1,
        bottom: 2,
        left: 0
    ...
    clean: function (element) { return d3.select(element).on("keydown", null).on("keyup", null).select("svg").remove(); },
    ...
    createGlass: function (svg, dimensions, handlers, axes, data, options, dispatch, columnWidth) {
        ...
        scrubberGroup = glass.selectAll(".scrubberItem").data(data).enter().append("g").attr("class", function (s, i) { return "scrubberItem series_" + i; });
        scrubberGroup.each(function (s, i) {
            ...
            that.styleTooltip(g.append("text").style("text-anchor", "start").attr({
                "class": function (d, i) { return "scrubberText series_" + i; },
                "height": "14px",
    ...
    getDataPerSeries: function (data, options) {
        ...
        straightened = series.map(function (s, i) {
            ...
            data.filter(function (row) { return row[s.y] != null; }).forEach(function (row) {
                var d;
                d = {
        ...
        layout = d3.layout.stack().values(function (s) { return s.values; });
    ...
    getWidestTickWidth: function (svg, axisKey) {
        ...
        if ((_ref = ticks[0]) != null) {
            _ref.forEach(function (t) { return max = Math.max(max, bbox(t).width); });
        }
        return max;
    },
    getWidestOrdinate: function (data, series, options) {
        var widest;
        widest = "";
        data.forEach(function (row) { return series.forEach(function (series) {
            var v, _ref;
            v = row[series.y];
